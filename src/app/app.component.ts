...
/*
  ...
  sendText() {
    const time = new Date();
    const message = this.messageService.convertToMorse(this.normalText.value);
    console.log(this.normalText.value);
    this.messageService.addMessage(time , message.trim()).then(done => {
      console.log('saved');
    }, err => {
      console.log(err);
    });
